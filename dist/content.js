...
(() => {
  // src/content.ts
  var MAX_SELECTION_WORDS = 30;
  var CONTEXT_RADIUS = 400;
  var trigger = null;
  var card = null;
  ...
  var TRIGGER_CLASSES = "define-it-trigger absolute z-[2147483647] rounded-full bg-neutral-900 text-neutral-50 text-xs font-semibold tracking-wide px-3 py-1.5 shadow-lg cursor-pointer border-0 hover:-translate-y-0.5 transition-transform font-sans";
  function showTrigger(selectionText, rect, context) {
    removeTrigger();
    removeCard();
  ...
  function createResultUI(term, body) {
  ...
  function showCard(rect, state, body, term) {
    removeCard();
    removeTrigger();
    card = document.createElement("div");
    card.className = "define-it-card absolute z-[2147483647] w-[380px] rounded-2xl bg-[#18181b] text-zinc-100 border border-zinc-700/60 shadow-2xl px-4 py-4 font-sans";
    if (state === "loading") {
      card.innerHTML = `
  ...
  function requestDefinition(term, context, rect) {
  ...
  document.addEventListener("mouseup", (e) => {
    if (e.target?.closest?.(".define-it-trigger, .define-it-card")) return;
